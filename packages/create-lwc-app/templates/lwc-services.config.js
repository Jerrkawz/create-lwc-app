...
module.exports = {
    resources: [{ from: 'src<% if (clientserver) { %>/client<% } %>/resources', to: 'dist/resources/' }<% if (slds) { %>,{from: 'node_modules/@salesforce-ux/design-system/assets', to: 'dist/resources/assets',}<% } %><% if ((appType === 'pwa' || bundler==='rollup') && !clientserver) { %>, { from: 'src/index.html', to: 'dist/'}, { from: 'src/manifest.json', to: 'dist/'}<% } %><% if (appType === 'pwa' && clientserver) { %>, { from: 'src/client/index.html', to: 'dist/index.html'}, { from: 'src/client/manifest.json', to: 'dist/manifest.json'}<% } %>],<% if (clientserver) { %>
